...
class Server(threading.Thread):
    def __init__(self, ip_addr, port_num, group_id, n_moves=len(ACTIONS) * NUM_MOVE_PER_ACTION):
        ...
        # the moves should be a random distribution. These haven't been randomly distributed yet
        # Generates a repeating list of 0 to n_moves-1. Repeats NUM_MOVE_PER_ACTION number of times.
        self.move_idxs = list(range(self.n_moves)) * NUM_MOVE_PER_ACTION
        # Check whether number of moves equals the number of actions times the number of moves per action.
        ...
        # Used to communicate from one thread to another. 
        ...
        self.socket.listen(1) # Only allows up to one incoming connection
        self.client_address, self.secret_key = self.setup_connection() 

    def decrypt_message(self, cipher_text):
        decoded_message = base64.b64decode(cipher_text)
        iv = decoded_message[:16] # Initialization vector?
        secret_key = bytes(str(self.secret_key), encoding="utf8") 

        cipher = AES.new(secret_key, AES.MODE_CBC, iv) # Create new cipher using the secret key
        decrypted_message = cipher.decrypt(decoded_message[16:]).strip() # Cipher is used to decrypt the message
        decrypted_message = decrypted_message.decode('utf8')

        decrypted_message = decrypted_message[decrypted_message.find('#'):] # Scplices with # as start
    ...
    def run(self):
        while not self.shutdown.is_set(): # Checking if event flag is set or not.
    ...
    def setup_connection(self):
         # Shuffle array, this will be used to give a random ordering of actions to perform
        random.shuffle(self.move_idxs)
        print("No actions for 60 seconds to give time to connect")
        # Will execute set_next_action after 60 seconds
        self.timer = threading.Timer(self.timeout, self.set_next_action)
        self.timer.start() # Will only execute once.
    ...
    def set_next_action(self):
        self.timer.cancel() # End timer thread
